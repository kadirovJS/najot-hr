...
import { ONBOARDING_TRACKS, type OnboardingTrack, getEmployeeOnboardingTrack } from '@/lib/onboarding';
...
const isAssigned = (video: VideoRecord, role: string, department: string) => {
  if (video.track) return video.track === getEmployeeOnboardingTrack(role, department);
...
export async function GET() {
  try {
    ...
    const trackSummaries = Object.fromEntries(ONBOARDING_TRACKS.map((track) => {
      const trackLearners = learners.filter((learner) => learner.track === track);
      return [track, {
        totalEmployees: trackLearners.length,
        averageCourseProgress: trackLearners.length ? Math.round(trackLearners.reduce((sum, learner) => sum + learner.courseProgress, 0) / trackLearners.length) : 0,
        completedCourses: trackLearners.filter((learner) => learner.assignedVideos > 0 && learner.courseProgress === 100).length,
      }];
    }));
